...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-causal
...
# ATTACH (`${VGI_CAUSAL_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run causal_worker.py`) — the
...
#                        `uv run --python 3.13 <repo>/causal_worker.py`.
# Optional:
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_CAUSAL_WORKER (the LOCATION) and, for the
...
case "$TRANSPORT" in
  subprocess)
    export VGI_CAUSAL_WORKER="$WORKER_CMD"
    ;;
  ...
  http)
    ...
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/causal-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/causal-http-server.log"
    ...
    export VGI_CAUSAL_WORKER="http://127.0.0.1:$PORT"
    ;;
  ...
  unix)
    ...
    SOCK="${TMPDIR:-/tmp}/causal-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/causal-unix-server.log"
    ...
    export VGI_CAUSAL_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_CAUSAL_WORKER) ..."
